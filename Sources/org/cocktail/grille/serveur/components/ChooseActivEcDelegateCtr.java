...
public class ChooseActivEcDelegateCtr extends ChooseEcDelegateCtr {
	public ChooseActivEcDelegateCtr(EOEditingContext ec,
	...
	public NSArray<EnsTreeItem> childrenTreeNodes(Object node) {
		if (!((childsNode.containsKey(node)) && (Boolean.TRUE.equals(childsNode
				.objectForKey(node))))) {
			boolean edithRight = false;
			NSArray<EnsTreeItem> tmp =  ((EnsTreeItem) node).getChilds();
			if ((tmp != null)
					&& (tmp.size() > 0)
					&& (tmp.lastObject().getEoObject() instanceof EOScolMaquetteEc)) {
				edithRight = true;
			}

			((EnsTreeItem) node).setChilds(ChooseActivUeDelegateCtr
					.filteredChilds(tmp, gUser(),
